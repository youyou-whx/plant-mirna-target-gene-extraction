...
BLACKLIST = {
    ...
    "UCL",
    "WRKY",
    "IIB",            # "P-type IIB Ca2+ ATPase" — protein type, not a gene
}
...
MIRNA_PATTERNS = [
    ...
    # Generic: miR156, miR156a, miR156a-5p, MIR398, miR444b.2
...
# Tier 3b: Mixed-case genes (HsfA1, Rht1) — require gene context
GENE_TIER3B = re.compile(r'\b([A-Z][a-z]{1,4}[A-Z]?\d{1,2})\b')
...
def extract_mirnas(text):
    """Return [(name, start, end), ...] — all occurrences."""
    results = []
    seen_pos = set()
...
def extract_genes(text):
    """Return [(name, confidence, start, end), ...] — all occurrences."""
    results = []
    seen_pos = set()
    ...
    for m in GENE_TIER1.finditer(text):
        name = m.group(0)
        key = (name, m.start())
        if key not in seen_pos and name not in BLACKLIST:
            seen_pos.add(key)
    ...
    for m in GENE_TIER2.finditer(text):
        ...
        key = (name, m.start())
        if key not in seen_pos and name not in BLACKLIST and not _is_generic(name):
            seen_pos.add(key)
    ...
    # Tier 3b: Mixed-case genes (HsfA1, Rht1) — context required
    for m in GENE_TIER3B.finditer(text):
        name = m.group(1)
        key = (name, m.start())
        if key in seen_pos:
            continue
        if name in BLACKLIST:
            continue
        if _has_gene_context(text, m.start(), m.end()):
            seen_pos.add(key)
            results.append((name, "Unverified", m.start(), m.end()))
...
def split_sentences(text):
    ...
    while i < n:
        if text[i] in '.!?':
            # .digit or .lower with no space → not a boundary (miR444b.2, e.g.)
            j = i + 1
            if j < n and (text[j].isdigit() or text[j].islower()):
                i = j
                continue
...
def pair_mirna_gene(mirnas, genes, text, abstract_body_text):
    """
    Pair miRNAs with genes. For each unique (miRNA, gene) pair, tries all
    position combinations, picks the closest one (prefer same-sentence).

    Returns: [{mirna, gene, gene_confidence, relation_type, sentence}, ...]
    """
    all_sentences = split_sentences(text)

    # Group positions by name: {name: [(start, end, sentence_index), ...]}
    mirna_pos = {}
    for m_name, m_start, m_end in mirnas:
        si = _find_sent_index(all_sentences, m_start)
        mirna_pos.setdefault(m_name, []).append((m_start, m_end, si))

    gene_pos = {}
    for g_name, g_conf, g_start, g_end in genes:
        si = _find_sent_index(all_sentences, g_start)
        gene_pos.setdefault(g_name, []).append((g_start, g_end, si, g_conf))

    pairs = []
    seen_pairs = set()

    for m_name, m_occurrences in mirna_pos.items():
        for g_name, g_occurrences in gene_pos.items():
            pair_key = (m_name, g_name)
            if pair_key in seen_pairs:
                continue

            # Find the best position pair — prefer same-sentence, then highest relation score
            best = None  # (sent_dist, rel_score, char_dist, ...)
            for m_s, m_e, m_si in m_occurrences:
                for g_s, g_e, g_si, g_conf in g_occurrences:
                    sd = abs(m_si - g_si)
                    cd = abs(m_s - g_s)
                    # Count relation signals in the span between miRNA and gene
                    si_lo, si_hi = min(m_si, g_si), max(m_si, g_si)
                    span_text = text[all_sentences[si_lo][0]:all_sentences[si_hi][1]].lower()
                    m_rel = sum(1 for sig in RELATION_SIGNALS if sig in span_text)
                    if best is None or (sd < best[0]) or (sd == best[0] and m_rel > best[1]):
                        best = (sd, m_rel, cd, m_s, m_e, g_s, g_e, g_conf, m_si, g_si)

            if best is None:
                continue
            sd, m_rel, cd, m_start, m_end, g_start, g_end, g_conf, m_si, g_si = best

            # Proximity check
            if sd == 0:
                pass
            elif sd == 1 and cd <= 400:
                pass
            else:
                continue
            ...
            sentence = _pick_best_sentence(
                text, all_sentences, m_si, g_si, m_start, m_end, g_start, g_end, m_name, g_name
            )
            ...
            mirnas_in_span = set()
            genes_in_span = set()
            for nm, ml in mirna_pos.items():
                for _, _, si in ml:
                    if si_min <= si <= si_max:
                        mirnas_in_span.add(nm)
            for gn, gl in gene_pos.items():
                for _, _, si, _ in gl:
                    if si_min <= si <= si_max:
...
def _pick_best_sentence(text, sentences, m_si, g_si, m_start, m_end, g_start, g_end, m_name, g_name):
    """
    Pick the sentence that best represents this specific miRNA–gene pair.
    Priority: sentences containing both → at least one → fallback.
    """
    if m_si == g_si:
        s, e = sentences[m_si]
        return text[s:e].strip()

    def _score(sent_text):
        return sum(1 for sig in RELATION_SIGNALS if sig in sent_text.lower())
    ...
    for si in range(min(m_si, g_si), max(m_si, g_si) + 1):
        ...
        if not sent_text:
            continue
        has_both = m_name in sent_text and g_name in sent_text
        has_gene = g_name in sent_text
        has_one = m_name in sent_text or has_gene
        if not has_one:
            continue
        score = _score(sent_text)
        # Prefer: both > gene-only (by score) > miRNA-only (by score)
        better = False
        if has_both and not best_has_both:
            better = True
        elif not has_both and best_has_both:
            better = False
        elif has_gene and not best_has_gene:
            better = True
        elif has_gene == best_has_gene and score > best_score:
            better = True
        if better or not best_sent:
            best_sent = sent_text
            best_score = score
            best_has_both = has_both
            best_has_gene = has_gene

    if best_sent:
        return best_sent

    # Fallback: gene's sentence
    s, e = sentences[g_si]
    return text[s:e].strip()
